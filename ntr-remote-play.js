...
const writeSocket = (socket, dBinaryPacket)=>{
    return new Promise((resolve,reject)=>{
...
async function NTRInitRemoteplay(sIp, bPriorityMode = 1, iPriorityFactor = 5, iQuality = 90, iQosValue = 20) {
    ...
    const iSocket = await openConnect(sPort, sIp);
    ...
    const socket = await openConnect(sPort, sIp);//NTR expects us to reconnect before it starts streaming frames
    console.log(`connected to ${sIp}:${sPort}`);
...
function NTRRemoteplayReadJPEG(listener) {
    const iSocket = dgram.createSocket('udp4');
    ...
    iSocket.on('message', (msg, rinfo) => {
        ...
        if (currentFrameId !== header.frameID) {
            ...
            frameList.push({ header, content: msg.slice(10) });
        }else{
            if (header.lastOne) {
                //assemble
                ...
                const jpeg = Buffer.from(jpegParts.join(''),'hex');
                const verify = jpeg.subarray(jpeg.byteLength-2).toString('hex').toUpperCase();
                if(verify!=="FFD9"){
                ...
                //cache
                frameList.push({ header, content: msg.subarray(10).toString('hex') });
            }
        }
    });
    ...
    iSocket.bind(8001);
    //   iSocket.setRecvBufferSize(2000);
}
...
exports.NTRInitRemoteplay = NTRInitRemoteplay;
exports.NTRRemoteplayReadJPEG = NTRRemoteplayReadJPEG;
